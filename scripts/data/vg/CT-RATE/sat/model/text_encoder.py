...
class Text_Encoder(nn.Module):
    def __init__(
        ...
        if checkpoint:
            ...
            checkpoint['model_state_dict'] = {k: v for k, v in checkpoint['model_state_dict'].items() if
                                              'atlas_tower' not in k and 'temperature' not in k}
            if partial_load:
                ...
                dummy = nn.Module()
                dummy.module = model
                dummy.load_state_dict(checkpoint['model_state_dict'])

        # open bert
        for name, param in model.named_parameters():
            if (
                'encoder.layer.' in name
                and int(name.split('encoder.layer.')[-1].split('.')[0]) > open_bert_layer
