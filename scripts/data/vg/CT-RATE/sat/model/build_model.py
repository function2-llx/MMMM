...
def load_checkpoint(
    ...
    if partial_load:
        ...
        unmatchd_state_dict = [k for k, v in checkpoint['model_state_dict'].items() if
        ...
        dummy.load_state_dict(model_dict)
...
def inherit_knowledge_encoder(
    ...
    visual_encoder_state_dict = {k.replace('atlas_tower', 'backbone'): v for k, v in
                                 checkpoint['model_state_dict'].items() if 'atlas_tower.encoder' in k}  # encoder部分
    model_dict.update(visual_encoder_state_dict)
    proj_state_dict = {k.replace('atlas_tower.', ''): v for k, v in checkpoint['model_state_dict'].items() if
                       'atlas_tower.projection_layer' in k}  # projection layer部分
