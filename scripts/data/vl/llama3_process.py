...
4. If such contents only describe interval changes relative to prior and whether the abnormalities are currently present cannot be definitely inferred, remove them entirely. For example, "cardiac and mediastinal silhouettes are stable", and "no new focal consolidation is seen" should be removed.
5. Keep the rest of the report exactly the same without any modification! Do not add any new contents.
Here is the input text for your task:
Input: {input}
...
def main():
    ...
    process(model, sampling_params, 'OpenI', ['test'])
